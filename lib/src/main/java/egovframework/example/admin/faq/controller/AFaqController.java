...
public class AFaqController {
	...
	public String insertFaq(AFaqVO vo, @RequestParam(name = "multifile") List<MultipartFile> multifile, HttpSession session) throws Exception {
		try {
	...
	public String updateFaq(AFaqVO vo, @RequestParam(name = "multifile") List<MultipartFile> multifile, HttpSession session) throws Exception {

		String userid = (String) session.getAttribute("userid");
		vo.setUserid(userid);

		try {
			AfaqDao.updateFaq(vo);

			fileService.insertFile(vo.getFaqId(), multifile);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String faqDelete(@RequestBody String param) throws Exception {
		...
		List<AFaqVO> faqVO = mapper.readValue(param, new TypeReference<List<AFaqVO>>() {});

		try {
			for (AFaqVO vo : faqVO) {
				int faqId = vo.getFaqId();
	...
	public String fileDeleteOne(@RequestParam(name = "fileName") String fileName, Model model) throws Exception {
